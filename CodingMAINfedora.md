...
> **Fedora variant:** derived from `CodingMAIN.md` (original template, unmodified) — adds the §10.6 Git policy (upstream pull-only rule). Read this file for sessions in the `CRS-fedora` repo.
...
- **`upstream`** → `https://github.com/FjingXR/class-replacement-system.git` (branch `Fjing`) — **PULL ONLY**. Never push code from this PC to that repo/branch, unless the user explicitly says so.
- Treat `upstream` as read-only: it exists solely to pull changes; no push commands to upstream ever.
- Always verify remote + branch before any push (`git remote -v && git branch --show-current`).

#### Branches
- Working branches in this repo: `fedora-backend`, `fedora-frontend` (plus `main`; origin tracks `fedora`, `fedora-backend`, `fedora-frontend`, `fedora-jing`)
